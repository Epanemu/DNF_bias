...
class Group(object):
    ...
    def return_f(self):
        return [
            self.func,
            self.group_size,
            self.weighted_disparity,
            self.disparity,
            self.disparity_direction,
            self.group_rate,
        ]
...
class Auditor:
    ...
    def __init__(self, X_prime, y, fairness_def):
        ...
        if self.fairness_def not in ["FP", "FN"]:
            raise Exception(
                "Invalid fairness metric specified: {}. Please choose 'FP' or 'FN'.".format(
                    self.fairness_def
                )
            )
        self.y = self.y_input
        ...
        for i in range(n):
            ...
            if self.y[i] == 0:
                new_group_cost = (
                    ...
                    * C
                    * (1.0 / iteration)
                    * (g_weight - g_members[X_prime_0_index])
                )
                ...
                if self.fairness_def == "FP":
                    c_1[i] = (
                        (c_1[i] - 1.0 / n) * ((iteration - 1.0) / iteration)
                        + new_group_cost
                        + 1.0 / n
                    )
                elif self.fairness_def == "FN":
                    c_0[i] = (
                        (c_0[i] - 1.0 / n) * ((iteration - 1.0) / iteration)
                        + new_group_cost
                        + 1.0 / n
                    )
    ...
    def get_subset(self, predictions):
        if self.fairness_def == "FP":
            ...
            return self.X_prime_0, [
                (1 - a) for u, a in enumerate(predictions) if self.y[u] == 0
            ]  # changed 1 to 0, inverting labels
    ...
    def get_group(self, predictions, metric_baseline):
        ...
        # print(np.unique(cost_1), metric_baseline)
        ...
        if sum(group_members_0) == 0:
            ...
            fp_group_rate = np.mean(
                [r for t, r in enumerate(predictions_subset) if group_members_0[t] == 1]
            )
        ...
        if sum(group_members_0_neg) == 0:
            ...
            fp_group_rate_neg = np.mean(
                [
                    r
                    for t, r in enumerate(predictions_subset)
                    if group_members_0_neg[t] == 1
                ]
            )
        ...
        if fp_disp_w_neg > fp_disp_w:
            return Group(
                func_neg, g_size_0_neg, fp_disp_w_neg, fp_disp_neg, -1, fp_group_rate
            )
        else:
