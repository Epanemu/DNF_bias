...
xlsx_path = 'h233.xlsx'
selected_cols_path = 'selected_cols.xlsx'
filtered_xlsx_path = 'filtered_data.xlsx'
interpreted_xlsx_path = 'interpreted_data.xlsx'
important_attributes = ['SEX', 'RACEV1X', 'HISPANX', 'TTLP21X', 'FAMINC21', 'HRWG53H', 
                        'POVCAT21', 'AGE21X', 'YRSINUS', 'BORNUSA', 'WHTLGSPK', 'INTVLANG', 'ADLANG42', 
                        'HWELLSPK', 'MARRY21X', 'EDUCYR', 'PERWT21F', 'INSCOPE', 'REGION21', 'HAVEUS42',
                        'SDPROBNONE', 'SDSTRESS', 'SDGETTGT', 
                        'SDDSCRMDR', 'SDDSCRMWRK', 'SDDSCRMJOB', 'SDDSCRMHS', 'SDDSCRMPOL', 'SDDSCRMPUB', 'SDDSCRMSTR',
                        'SDPUBTRANS', 'ANYLMI21', 'INSCOV21']
#-----------------
...
def get_filtered_data():
    ...
    conditions = [
        (df['SDDSCRMDR'] == 1) | (df['SDDSCRMWRK'] == 1) | (df['SDDSCRMJOB'] == 1) | (df['SDDSCRMHS'] == 1) | (df['SDDSCRMPOL'] == 1) | (df['SDDSCRMPUB'] == 1) | (df['SDDSCRMSTR'] == 1),
        (df['SDDSCRMDR'] == 2) | (df['SDDSCRMWRK'] == 2) | (df['SDDSCRMJOB'] == 2) | (df['SDDSCRMHS'] == 2) | (df['SDDSCRMPOL'] == 2) | (df['SDDSCRMPUB'] == 2) | (df['SDDSCRMSTR'] == 2),
        (df['SDDSCRMDR'] == -15) | (df['SDDSCRMWRK'] == -15) | (df['SDDSCRMJOB'] == -15) | (df['SDDSCRMHS'] == -15) | (df['SDDSCRMPOL'] == -15) | (df['SDDSCRMPUB'] == -15) | (df['SDDSCRMSTR'] == -15),
    ]
    choices = [1, 2, -15]
    df['SDDCOMB'] = np.select(conditions, choices, default=-1)
    # OR logical function for all 7 columns

    df.drop(columns=['SDDSCRMDR', 'SDDSCRMWRK', 'SDDSCRMJOB', 'SDDSCRMHS', 'SDDSCRMPOL', 'SDDSCRMPUB', 'SDDSCRMSTR'], inplace=True)
...
def interpret_data():
    ...
    inscope_mapping = {1: 'INSCOPE AT SOME TIME DURING 2021', 2: 'OUT-OF-SCOPE FOR ALL OF 2021'}
